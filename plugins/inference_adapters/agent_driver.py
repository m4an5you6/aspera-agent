...
def build_inference_agent_prompt(job_spec: Dict[str, Any], inference_spec: Dict[str, Any]) -> str:
    ...
    role_block = (
        ...
        "(master_url=http://<master>:8765 + GPUCLOUD_CLUSTER_SECRET only; "
        "never Ray ports)\n"
        ...
        "HARD: start_vllm refuses if `ray status` GPU count < tensor_parallel.\n"
        ...
        if nnodes > 1 and node_rank == 0
            ...
            "1) Call `inference_ray_join` ONCE to ray head address "
            "(head advertised_addr:head_port). The tool polls forever by default "
            "until the head is up — rank0 may still be installing deps. "
            "Do NOT abort after a timeout/error and do NOT invent your own "
            "retry loop with terminal; wait for the tool to return success.\n"
            "2) Only after join success: `inference_report_ready` with "
            "success=true and details.phase=worker_ready. "
            "FORBIDDEN: report worker_ready if join failed or timed out.\n"
    ...
    return (
        ...
        "Transformers fallback is not a valid plan). Preferred cu tag when "
        "arch allows; if the chosen vLLM requires a newer torch/cu tag "
        "(e.g. 0.17 → torch 2.10 cu128), plan an intentional dedicated venv "
        "+ exact torch pin + CUDA smoke — do not pre-BLOCK on nvidia-smi "
        "CUDA 12.4 alone, and never --no-deps-mix vLLM with an older torch. "
        "Only cite driver insufficient / PTX after smoke or serve logs show it. "
