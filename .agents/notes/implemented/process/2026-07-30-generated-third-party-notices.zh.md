...
文件默认只披露**直接**依赖；完整的 npm 与 Python SDK 闭包保留在各自锁文件中。`@anthropic-ai/claude-agent-sdk` 声明的官方 Claude 可执行包会单独列出。`python/*/pyproject.toml` 中的直接 Python SDK 依赖出现在自有表格中，许可证元数据记录在生成器内；元数据缺失会使生成失败。这些分发包同样通过运行时许可证检查，其中包括 Pillow 与 typing-extensions 声明的宽松许可证标识 `MIT-CMU` 和 `PSF-2.0`。
