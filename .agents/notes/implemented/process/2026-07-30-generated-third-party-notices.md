...
The file discloses **direct** dependencies by default; complete npm and Python SDK closures remain in their lock files. The official Claude executable packages declared by `@anthropic-ai/claude-agent-sdk` receive explicit entries. Direct Python SDK dependencies named by `python/*/pyproject.toml` appear in their own table, with license metadata recorded in the generator; missing metadata fails generation. Those distributions also pass the runtime license check, including the permissive `MIT-CMU` and `PSF-2.0` identifiers declared by Pillow and typing-extensions.
